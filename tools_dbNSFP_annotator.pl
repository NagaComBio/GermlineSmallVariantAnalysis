...
if (@ARGV < 1)
{
	die "$0 needs at least 1 argument: 1) vcf file with SNVs to add dbNSFP entries to - optional 2) path to the bgzipped, tabix indexed dbNSFP database file (default: ./dbNSFP/dbNSFP2.9_variant_all_plain.gz)\n";
}


my $inputfile = $ARGV[0];
my $database = $ARGV[1] ||  "./dbNSFP/dbNSFP2.9_variant_all_plain.gz";
my $cmd; # tabix call;
unless (-r $database)
{
...
close $EING;
print STDERR "From $counter entries in SNV file, $present have a dbNSFP entry.\n";
exit;
